...
function isModernUserAgentPreset(preset) {
  return Boolean(
    USER_AGENT_PRESET_DEFINITIONS.find(
      (definition) => definition.preset === preset && definition.modern,
    ),
  );
}
...
function isSafariUserAgentPreset(preset) {
  return Boolean(
    USER_AGENT_PRESET_DEFINITIONS.find(
      (definition) => definition.preset === preset && definition.family === "safari",
    ),
  );
}

function isChromiumUserAgentPreset(preset) {
  return USER_AGENT_PRESET_DEFINITIONS.some(
    (definition) =>
      definition.preset === preset && definition.family !== "safari",
  );
}
...
function parseUserAgentSelection(value) {
  ...
  if (!USER_AGENT_PRESET_DEFINITIONS.some((entry) => entry.preset === preset)) {
...
function getUserAgentSelectionValue(catalog, preset, curlProfile = "auto") {
  ...
  const definition = USER_AGENT_PRESET_DEFINITIONS.find(
...
function getUserAgentSelectionOptions(
  ...
  if (!options.some((option) => option.value === currentValue)) {
    const definition = USER_AGENT_PRESET_DEFINITIONS.find(
      (entry) => entry.preset === selectedUserAgent.preset,
    );
    if (definition) {
      options.push({
        value: currentValue,
...
function createCurlProfile(
  ...
  return {
    ...
    navigator: {
      ...
      oscpu,
      vendor: "Google Inc.",
      hardwareConcurrency: mobile ? 8 : 8,
      deviceMemory: 8,
