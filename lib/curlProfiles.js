...
function isCurlProfileCompatible(preset, curlProfile) {
  const definition = getUserAgentDefinition(preset);
  if (!definition || typeof curlProfile !== "string") return false;
  if (curlProfile === "auto") return true;

  const match = curlProfile.match(
    definition.family === "safari"
      ? /^safari\d+(_ios)?$/
      : definition.family === "chrome"
        ? /^chrome\d+(_android)?$/
        : /^edge\d+$/,
  );
  return Boolean(match && Boolean(match[1]) === definition.mobile);
}
...
function parseUserAgentSelection(value) {
  ...
  return {
    preset,
    curlProfile: isCurlProfileCompatible(preset, curlProfile)
      ? curlProfile
      : "auto",
  };
}
...
function getCurlProfileEntries(catalog) {
  return normalizeCurlProfileCatalog(catalog).profiles;
}
...
function createCurlProfileFromWrapper(name, source, updatedAt = Date.now()) {
  ...
  if (!isModernCurlProfileTarget(target)) return null;
...
function createCurlProfileVariant(profile, preset) {
  ...
  const variant = createCurlProfile(profile.target, profile.version, {
    mobile,
...
function getCurlProfileForConfig(config, catalog) {
  ...
  const definition = getUserAgentDefinition(userAgent.preset);
  if (!definition) return null;
  const entries = getCurlProfileEntries(catalog).filter((entry) =>
    entry.family === definition.family && Boolean(entry.mobile) === definition.mobile,
  );
  const profile = entries.find((entry) => entry.target === userAgent.curlProfile) || entries[0];
  return definition.family === "safari"
    ? profile
    : createCurlProfileVariant(profile, definition.preset);
}
