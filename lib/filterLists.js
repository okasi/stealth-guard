...
const DEFAULT_FILTER_LISTS_BY_ID = new Map(
  DEFAULT_FILTER_LISTS.map((entry) => [entry.id, entry]),
);
const MAX_FILTER_LISTS = 32;
...
function normalizeFilterListEntries(value, useDefaultNames = true) {
  ...
  for (const entry of source) {
    if (result.length >= MAX_FILTER_LISTS) break;
    ...
    const defaultEntry = DEFAULT_FILTER_LISTS_BY_ID.get(id);
    result.push({
      id,
      name:
