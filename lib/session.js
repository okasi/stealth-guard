...
function createSessionManager({ storageApi, browserApi, callApi, warn = () => {} }) {
  const runtimeApi =
    typeof createSerialQueue === "function"
      ? { createSerialQueue, mapWithConcurrency }
      : require("./runtime.js");
  const enqueue = runtimeApi.createSerialQueue();
  const mapCookies = (cookies, operation) =>
    runtimeApi.mapWithConcurrency(cookies, 8, operation);

  // V8 does not credit implicit fallthrough branches inside these queued async
  // operations. The directives below ignore only those empty branches; direct
  // tests exercise both every guarded error and its continuation path.
  ...
  function getHostname(url) {
    return (
      typeof getHostnameFromUrl === "function"
        ? getHostnameFromUrl(url)
        : require("./domainFilter.js").getHostnameFromUrl(url)
    ) || "";
  }
  ...
  async function clearCookies(hostname, tabId) {
    ...
    await mapCookies(cookies, async (cookie) => {
      try {
        await callApi(
          browserApi.cookies,
          "remove",
  ...
  function sessionsForHostname(sessions, hostname) {
    return sessions
      ...
      .sort(
        /* c8 ignore next -- V8 misses the tested left fallback inside Array.sort. */
        (left, right) =>
