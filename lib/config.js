...
const DEFAULT_TRACKER_FILTER_LISTS = Object.freeze([
  Object.freeze({
    id: "adguard-base",
...
function normalizeTrackerFilterLists(value) {
  const source = Array.isArray(value) ? value : DEFAULT_TRACKER_FILTER_LISTS;
  const result = [];
  const seen = new Set();
  for (const entry of source) {
    ...
    const id =
      typeof entry.id === "string"
        ...
        : "";
    const name =
      typeof entry.name === "string" ? entry.name.trim().slice(0, 128) : "";
  ...
  return result.length
    ? result
    : DEFAULT_TRACKER_FILTER_LISTS.map((entry) => ({ ...entry }));
}
...
function normalizeConfig(config) {
  ...
  normalized.tracker = {
    ...
    filterLists: normalizeTrackerFilterLists(trackerSource.filterLists),
    customFilters:
...
function createContentConfig(config, hostname = "") {
  ...
  contentConfig.adblock = {
    enabled: Boolean(
      normalized.enabled &&
        normalized.tracker.enabled &&
        !matches(hostname, normalized.globalWhitelist) &&
        !matches(hostname, normalized.tracker.whitelist)
    ),
