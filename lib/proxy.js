 ...
 */
function generatePACScript(profiles, routes, defaultProfile, globalWhitelist) {
  const pacEscape = (value) => String(value).replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
  const allowlistPatterns = (globalWhitelist || "")
    .split(",")
    .map(s => s.trim().toLowerCase())
    .filter(Boolean);

  const buildAllowlistConditions = () => {
    if (allowlistPatterns.length === 0) {
      return "";
    }

    const checks = allowlistPatterns.map(pattern => {
...
${buildAllowlistConditions()}`;
  ...
  routes.forEach(route => {
    ...
    if (pattern.startsWith('*.')) {
      ...
      pac += `  if (host === "${domain}" || host.endsWith(".${domain}")) {\n`;
      ...
      pac += `  // Route ${pattern} -> ${profile.name}\n`;
      pac += `  if (host.startsWith("${prefix}.")) {\n`;
      pac += `    return "${proxyString}";\n`;
      pac += `  }\n\n`;
    } else {
      // Exact match (also match www. variant)
      pac += `  // Route ${pattern} -> ${profile.name}\n`;
      pac += `  if (host === "${pattern}" || host === "www.${pattern}") {\n`;
      pac += `    return "${proxyString}";\n`;
      pac += `  }\n\n`;
    }
  });
...
async function applyProxySettings() {
  ...
  if (routes.length === 0 && activeProfile && !hasGlobalWhitelist) {
    ...
    if (config.proxy.bypassList && config.proxy.bypassList.length > 0) {
      proxyConfig.rules.bypassList = config.proxy.bypassList;
    }
  ...
  const pacScript = generatePACScript(profiles, routes, activeProfile, config.globalWhitelist);
  await chrome.proxy.settings.set({
    value: {
      mode: 'pac_script',
      pacScript: {
