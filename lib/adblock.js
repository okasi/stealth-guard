...
const DEFAULT_FILTER_SUBSCRIPTIONS = Object.freeze([
  Object.freeze({
    id: "adguard-base",
...
function normalizeFilterSubscriptions(value) {
  const defaultsById = new Map(
    DEFAULT_FILTER_SUBSCRIPTIONS.map((entry) => [entry.id, entry]),
  );
  const source = Array.isArray(value) ? value : DEFAULT_FILTER_SUBSCRIPTIONS;
  const seen = new Set();
  const result = [];
  ...
  return result.length
    ? result
    : DEFAULT_FILTER_SUBSCRIPTIONS.map((entry) => ({ ...entry }));
}
...
function normalizeRuleDomains(value) {
  ...
  for (const raw of value.split("|")) {
    ...
    (negated ? exclude : include).push(domain.replace(/^\*\./, ""));
  }
...
function domainMatches(hostname, domain) {
  ...
  if (domain && domain.includes("*")) {
    const escaped = domain
      .replace(/[.+?^${}()|[\]\\]/g, "\\$&")
      .replace(/\*/g, "[^.]+");
    return Boolean(normalized && new RegExp(`(?:^|\\.)${escaped}$`, "i").test(normalized));
  }
  return Boolean(
    normalized &&
      domain &&
...
function ruleOptionsMatch(rule, details, pageHostname, requestHostname) {
  ...
  const thirdParty = !domainMatches(requestHostname, pageHostname) &&
    !domainMatches(pageHostname, requestHostname);
  if (options.thirdParty !== null && options.thirdParty !== thirdParty) return false;
  if (
    options.domains.length &&
    !options.domains.some((domain) => domainMatches(pageHostname, domain))
  ) return false;
  if (options.excludedDomains.some((domain) => domainMatches(pageHostname, domain))) {
    return false;
  }
  return true;
}

function candidateRules(index, url, requestHostname) {
  const result = new Set(index.fallback);
  for (const suffix of hostnameSuffixes(requestHostname)) {
    for (const rule of index.domains.get(suffix) || []) result.add(rule);
  }
  for (const token of String(url || "").toLowerCase().match(/[a-z0-9_%.-]{4,}/g) || []) {
    for (const rule of index.tokens.get(token) || []) result.add(rule);
  }
  return result;
}

function indexMatches(index, details, pageHostname, requestHostname, importantOnly = false) {
  for (const rule of candidateRules(index, details.url, requestHostname)) {
    if (importantOnly && !rule.options.important) continue;
    if (!ruleOptionsMatch(rule, details, pageHostname, requestHostname)) continue;
    if (rule.kind === "domain" || (rule.regex && rule.regex.test(details.url))) {
      return true;
    }
  }
  return false;
...
function shouldBlockRequest(engine, details, pageHostname, requestHostname) {
  ...
  if (indexMatches(engine.block, details, pageHostname, requestHostname, true)) {
    return true;
  }
  if (indexMatches(engine.allow, details, pageHostname, requestHostname)) return false;
  return indexMatches(engine.block, details, pageHostname, requestHostname);
}
