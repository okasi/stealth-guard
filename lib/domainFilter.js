...
const PARSED_WHITELIST_CACHE_MAX = 256;
const WILDCARD_REGEX_CACHE_MAX = 256;
const parsedWhitelistCache = new Map();
...
function parseWhitelistPatterns(whitelistString) {
  const cacheKey = typeof whitelistString === "string" ? whitelistString : "";
  if (parsedWhitelistCache.has(cacheKey)) {
    return parsedWhitelistCache.get(cacheKey);
  }

  const patterns = cacheKey
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);

  cacheSetWithLimit(parsedWhitelistCache, cacheKey, patterns, PARSED_WHITELIST_CACHE_MAX);
  return patterns;
}
...
function getWildcardRegex(pattern) {
  ...
  cacheSetWithLimit(wildcardRegexCache, pattern, wildcardRegex, WILDCARD_REGEX_CACHE_MAX);
...
class DomainFilter {
  ...
  isWhitelisted(hostname, whitelistString) {
    ...
    const patterns = parseWhitelistPatterns(whitelistString);

    // Check each pattern
    for (const pattern of patterns) {
      if (this.matchesPattern(normalizedHostname, pattern, true)) {
        return true;
      }
    }
   ...
   */
  matchesPattern(hostname, pattern, isNormalized = false) {
    if (!isNormalized) {
  ...
  addDomainToWhitelist(domain, whitelistString) {
    ...
    // Check if already exists or is already covered by a wildcard pattern.
    if (this.isDomainInWhitelist(domain, whitelistString) || this.isWhitelisted(domain, whitelistString || "")) {
      return whitelistString;
    }
  ...
  removeDomainFromWhitelist(domain, whitelistString) {
    ...
    // Remove every rule that currently covers the requested domain. This makes
    // UI toggles work when the domain is covered by a parent or generic wildcard.
    const filtered = domains.filter(d => {
      const normalized = d.toLowerCase();
      return !this.matchesPattern(targetDomain, normalized, true);
    });
